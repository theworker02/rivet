...
Rivet `1.2.0` commands are executable and covered by integration or end-to-end tests:
...
## Configuration, diagnostics, and output

`rivet init` and `rivet config migrate` write dependency-free JSON. The root marker is `"rivet": 2`; legacy v1 documents are migrated explicitly rather than silently reinterpreted. YAML manifests in `examples/` are reference inputs and are not parsed by core startup without a maintained adapter.

`doctor`, `preflight`, `pulse`, `compatibility`, `resource-test`, and `benchmark runtime` are backed by the runtime, health, resource, and capability modules. They are not decorative command routes: each returns structured output and is exercised by verification or integration coverage. Use `--json` on commands that provide it for shell automation; otherwise the human report remains intentionally concise and bounded.
